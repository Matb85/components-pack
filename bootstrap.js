...
function addPhotos(photos = document.querySelectorAll('.MP-photo:not(.MP-dont-load) > img')) {
    photos.forEach((img) => {
        if (!img.dataset?.prevent?.includes('addtolist'))
            img.addEventListener('load', () => mutations.addPhoto(window.astroPack, {
        ...
        parent.addEventListener('click', () => {
            ...
            window.dispatchEvent(new CustomEvent(enlarger, { detail: { rect: parent.getBoundingClientRect(), img } }));
        });
    });
}
window.addEventListener('load', () => addPhotos());
window.addEventListener('MP-addPhotos', (e) => addPhotos(e.detail));
